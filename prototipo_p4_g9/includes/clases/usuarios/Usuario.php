...
class Usuario
{
    ...
    public static function getBistrocoins($idUsuario){
        $conn = Aplicacion::getInstance()->getConexionBd();
        $stmt = $conn->prepare("SELECT bistrocoins FROM usuarios WHERE id = ?");
        $stmt->bind_param("i", $idUsuario);
        $stmt->execute();
        $row = $stmt->get_result()->fetch_assoc();

        return $row ? (int)$row['bistrocoins'] : 0;
    }

    public static function sumaBistrocoins($idUsuario, $cantidad){
        $conn = Aplicacion::getInstance()->getConexionBd();
        $stmt = $conn->prepare("UPDATE usuarios SET bistrocoins = bistrocoins + ? WHERE id = ?");
        $stmt->bind_param("ii", $cantidad, $idUsuario);
        return $stmt->execute();
    }

    public static function restaBistrocoins($idUsuario, $cantidad){
        $conn = Aplicacion::getInstance()->getConexionBd();
        $stmt = $conn->prepare("UPDATE usuarios SET bistrocoins = bistrocoins - ? WHERE id = ? AND bistrocoins >= ?");
